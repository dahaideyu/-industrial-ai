...
class DeepSeekService:
    ...
    async def chat(self, system_prompt: str, user_prompt: str) -> Tuple[Optional[str], Optional[str]]:
        """调用DeepSeek聊天，返回 (content, error_message)"""
        ...
        logger.info(f"[DeepSeek] 调用模型 - model: {self.model}")
        logger.debug(f"[DeepSeek] System Prompt: {system_prompt}")
        logger.debug(f"[DeepSeek] User Prompt: {user_prompt}")
        try:
            async with httpx.AsyncClient(timeout=self.timeout) as client:
                response = await client.post(url, headers=self._get_headers(), json=payload)
                logger.info(f"[DeepSeek] 响应 - status: {response.status_code}")
                if response.status_code == 200:
                    data = response.json()
                    content = data.get("choices", [{}])[0].get("message", {}).get("content", "")
                    logger.debug(f"[DeepSeek] 输出内容: {content}")
                    return content, None
                else:
                    err_msg = f"DeepSeek调用失败: {response.status_code} - {response.text}"
                    logger.error(f"[DeepSeek] {err_msg}")
                    return None, err_msg
        except Exception as e:
            err_msg = f"DeepSeek调用异常: {str(e)}"
            logger.exception(f"[DeepSeek] {err_msg}")
            return None, err_msg
    ...
    async def generate_repair_suggestion(
        ...
        logger.info(f"[DeepSeek] 生成维修建议 - device: {device_name}, device_type: {device_type}")
        logger.debug(f"[DeepSeek] 故障描述: {fault_description}")
        logger.debug(f"[DeepSeek] 有历史维修单: {bool(history_content and history_content.strip())}")
        logger.debug(f"[DeepSeek] 有文档内容: {bool(doc_content and doc_content.strip())}")
        ...
        logger.info("[DeepSeek] 第一步：生成故障处理总结")
        if has_knowledge:
...
请直接输出总结内容，不要包含其他多余内容，控制在150-200字之间。"""
        ...
        logger.info("[DeepSeek] 第二步：生成详细维修建议")
        if has_knowledge:
...
每项标题必须加粗，请使用中文回答。"""
    ...
    async def detect_spare_replacement(self, handle_action: str) -> Tuple[bool, Optional[str]]:
        ...
        """
        logger.info(f"[DeepSeek] 检测备件更换情况")
        logger.debug(f"[DeepSeek] 处理措施: {handle_action}")
...
只返回JSON，不要其他文字。"""
        ...
        try:
            ...
            logger.info(f"[DeepSeek] 备件检测结果 - has_replacement: {has_replacement}, detail: {detail}")
    ...
    async def generate_task_suggestion(
        ...
        logger.info(f"[DeepSeek] 生成任务建议 - procedure: {procedure}, task_type: {task_type}")
        logger.debug(f"[DeepSeek] 任务描述: {task_description}")
        logger.debug(f"[DeepSeek] 有文档内容: {bool(doc_content and doc_content.strip())}")

        has_doc = bool(doc_content and doc_content.strip())

        # 第一步：生成 conclusion（总结）
        logger.info("[DeepSeek] 第一步：生成任务建议总结")
        if has_doc:
...
请直接输出总结内容，不要包含其他多余内容，控制在150-200字之间。"""
        ...
        # 第二步：生成 context（详细建议）
        logger.info("[DeepSeek] 第二步：生成详细任务改善建议")
        if has_doc:
...
处理措施：{}""".format(task_description, handle_action)
    ...
    async def score_content_quality(
        ...
        """
        logger.info(f"[DeepSeek] 内容质量评分 - is_replace_spare: {is_replace_spare}")
        logger.debug(f"[DeepSeek] 根因分析: {handle_analysis}")
        logger.debug(f"[DeepSeek] 处理措施: {handle_action}")
...
请以JSON格式返回，格式如下：
{{}}
只返回JSON，不要其他文字。""".format(max_score, "score")
