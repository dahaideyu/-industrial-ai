...
def ensure_device_info_schema(pg):
    """device_info.device_id 建表脚本(01_schema_postgres.sql)里是 SERIAL(integer)，
    但真实设备编码是 'BTR-QSDLQM-01-001' 这类字符串——integer 列既存不下设备编码，
    也会让 device_energy_meter 的 varchar 外键建不起来。与 sync_business_tables.py
    的同名函数保持一致，自动纠正为 VARCHAR(64)，幂等。"""
    with pg.cursor() as cur:
        cur.execute(
            """
