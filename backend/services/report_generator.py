...
class ReportGenerator:
    ...
    def __init__(self, llm_model: str = None, base_url: str = None, api_key: str = None, max_workers: int = 3):
        """
        初始化报告生成器
        ...
        # 如果没有传入参数，则根据 PROVIDER 环境变量动态读取配置
        if not base_url or not api_key:
            ...
            base_url = base_url or os.getenv(base_url_var)
            api_key = api_key or os.getenv(api_key_var)
            default_model = os.getenv(model_var, default_model)
        ...
        # 模型配置缓存：{report_code: {"base_url": str, "api_key": str, "model": str, "llm": OpenAI}}
    ...
    def _load_model_config_file(self) -> dict:
        ...
        if not os.path.exists(config_path):
            print(f"[模型配置] 配置文件不存在: {config_path}，使用默认模型配置")
            return {}

        try:
            with open(config_path, 'r', encoding='utf-8') as f:
                config = yaml.safe_load(f)
                print(f"[模型配置] 已加载配置文件: {config_path}")
                return config or {}
        except Exception as e:
            print(f"[模型配置] 加载配置文件失败: {e}，使用默认模型配置")
            return {}
    ...
    def _get_llm_config_for_report(self, report_code: str) -> dict:
        """
        根据 report_code 获取对应的 LLM 配置

        配置优先级：
          1. report_models 中指定的配置
          2. .env 中 PROVIDER 对应的配置（默认）

        Args:
            report_code: 报告类型编码

        Returns:
            {"base_url": str, "api_key": str, "model": str, "is_ollama": bool}
        """
        # 检查缓存
        ...
        # 1. 从 report_models 中获取配置
        if report_code in report_models:
            report_config = report_models[report_code]
            if isinstance(report_config, dict):
                # 如果配置了 provider，先从环境变量读取
                if 'provider' in report_config:
                    config = self._resolve_provider_config(report_config['provider'])
                # 直接指定的参数覆盖 provider 读取的值
                if 'base_url' in report_config:
                    config['base_url'] = report_config['base_url']
                if 'api_key' in report_config:
                    config['api_key'] = report_config['api_key']
                if 'model' in report_config:
                    config['model'] = report_config['model']

        # 2. 未配置的字段使用实例默认配置（来自 .env）
        ...
        # 如果是 Ollama，修正 base_url
        if config['is_ollama']:
            config['base_url'] = self._normalize_ollama_url(config['base_url'])

        # 缓存配置
        self._model_config_cache[report_code] = config
    ...
    def _ollama_chat(self, prompt: str, llm_config: dict = None) -> str:
        ...
        print(f"[Ollama] 原生 API 非流式调用: {url}")
        print(f"[Ollama] 参数: model={model}, temperature=0.2, think={think}")
    ...
    def _ollama_stream(self, prompt: str, llm_config: dict = None) -> Generator[str, None, None]:
        ...
        print(f"[Ollama] 原生 API 流式调用: {url}")
        print(f"[Ollama] 参数: model={model}, temperature=0.2, think={think}")
    ...
    def generate_reports(
        ...
        # 获取当前报告类型的模型配置
        llm_config = self._get_llm_config_for_report(report_code)

        log = job_logger.info if job_logger else print

        log(f"\n{'='*60}")
        log(f"[报告生成] 共 {len(templates)} 份报告待生成")
        log(f"[报告生成] 使用模型: {llm_config.get('model', '未知')}")
        ...
        if parallel and len(templates) > 1:
            # 并行生成
            results = self._generate_reports_parallel(data, templates, job_logger, llm_config)
        else:
            # 串行生成
            results = self._generate_reports_sequential(data, templates, job_logger, llm_config)
    ...
    def generate_reports_stream(
        ...
        if job_logger:
            ...
            job_logger.info("[报告生成] 使用模型: %s", llm_config.get('model', '未知'))
        ...
        for index, template_config in enumerate(templates):
            ...
            try:
                ...
                # 构建提示词
                # ========================================
                log = job_logger.info if job_logger else lambda m, *a: print(m % a)
...
{}""".format(prompt, context)
                ...
                if llm_config.get('is_ollama'):
                    # Ollama 原生 API 流式调用
                    if job_logger:
                        job_logger.info("[报告 %d/%d] 使用 Ollama 原生 API /api/chat, model=%s", index + 1, total_count, llm_config.get('model'))
                    for content in self._ollama_stream(prompt, llm_config):
                    ...
                    # OpenAI 兼容接口
                    if job_logger:
                        job_logger.info("[报告 %d/%d] 使用 OpenAI 兼容接口, model=%s", index + 1, total_count, llm_config.get('model'))
                    for chunk in llm_client.chat.completions.create(
                        ...
                        # 部分 API（如 opencode）的流式响应中某些 chunk 的 choices 可能为空
    ...
    def generate_report_with_citation(
        ...
        prompt = report_prompt.replace('{data_sources}', data_json)
        print(f"  原始数据长度: {len(data_json)} 字符")
        ...
        if use_ragflow:
            ...
            if retrieved_chunks:
                ...
                print(f"[报告生成] 检索到 {len(retrieved_chunks)} 个知识块，已注入提示词")
            else:
                print(f"[报告生成] 未检索到相关知识块，使用普通 LLM 生成")
...
【基础质量周报】
{}""".format(prompt, context)
            print(f"  添加背景上下文: {len(context)} 字符")
...
【上一期基础报告】
{}""".format(prompt, historical_context)
            print(f"  添加历史上下文: {len(historical_context)} 字符")

        print(f"  知识库检索: {len(citations)} 个知识块")
        for i, cite in enumerate(citations[:3]):
            print(f"    [{i+1}] {cite['source']} (相似度: {cite['similarity']:.3f})")

        print(f"\n  最终提示词总长度: {len(prompt)} 字符")
        print(f"{'='*70}\n")

        # 使用配置的模型或默认模型
        if llm_config:
            model = llm_config.get('model', self.model)
            is_ollama = llm_config.get('is_ollama', False)
            llm_client = self._get_llm_client(llm_config)
        else:
            model = self.model
        ...
        # 调用 LLM 生成报告
        print(f"[单报告] 开始调用 LLM 生成...")
        if is_ollama:
            # Ollama 原生 API 非流式调用
            print(f"[单报告] 使用 Ollama 原生 API /api/chat, model={model}")
            content = self._ollama_chat(prompt, llm_config)
        else:
            # OpenAI 兼容接口
            print(f"[单报告] 使用 OpenAI 兼容接口, model={model}")
    ...
    def _build_search_query(self, data: dict, context: str = None, llm_config: dict = None) -> str:
        ...
        if context and len(context) > 100:
            try:
                ...
                if search_query:
                    print(f"[检索查询] 使用 LLM 提取的关键词: {search_query[:200]}...")
                    return search_query
            except Exception as e:
                print(f"[检索查询] LLM 提取关键词失败，回退到基础提取: {e}")
        ...
        fallback = ' '.join(parts)
        print(f"[检索查询] 极简回退: {fallback if fallback else '质量检验数据'}")
...
请直接输出关键词（用空格分隔）：""".format(report_content[:2000])
    ...
    def _retrieve_with_segmentation(
        ...
        print(f"[分段检索] 报告内容分为 {len(segments)} 段")
        ...
        for idx, segment in enumerate(segments):
            ...
            print(f"[分段检索] 检索第 {idx+1}/{len(segments)} 段: {segment[:50]}...")
            ...
            try:
                ...
                if chunks:
                    ...
                    print(f"[分段检索] 找到 {len(chunks)} 个相关块")

            except Exception as e:
                print(f"[分段检索] 检索段落失败: {e}")
                continue

        print(f"[分段检索] 总共找到 {len(all_chunks)} 个相关块")
...
""".format(chunk_count, knowledge_context)
    ...
    def _generate_reports_sequential(self, data: dict, templates: List[Dict], job_logger=None, llm_config: dict = None) -> List[Dict]:
        """
        串行生成报告
        ...
        for template_config in templates:
            ...
            try:
                ...
                if use_ragflow:
                    ...
                    content = self._generate_report(data, template_file, llm_config)
                    citations = []
                ...
                print(f"[报告生成] {output_name} 生成完成")
                print(f"[报告生成] 内容长度: {len(content)} 字符")
                print(f"[报告生成] 引用数量: {len(citations)}")

            except Exception as e:
                import traceback
                traceback.print_exc()
                print(f"[错误] {output_name} 生成失败: {e}")
                results.append({
                    'output_name': output_name,
    ...
    def _generate_reports_parallel(self, data: dict, templates: List[Dict], job_logger=None, llm_config: dict = None) -> List[Dict]:
        """
        并行生成报告
        ...
        with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            # 提交所有任务
            future_to_template = {
                executor.submit(self._generate_single_report, data, template, job_logger, llm_config): template
            ...
            for future in as_completed(future_to_template):
                ...
                try:
                    ...
                    completed_count += 1
                    print(f"\n[进度] {completed_count}/{total_count} 完成 - {result['output_name']}")
    ...
    def _generate_single_report(self, data: dict, template_config: Dict, job_logger=None, llm_config: dict = None) -> Dict:
        """
        生成单份报告
    ...
    def _generate_report(self, data: dict, template_file: str, llm_config: dict = None) -> str:
        """
        生成报告
        ...
        if is_ollama:
            # Ollama 原生 API 非流式调用
            print(f"[_generate_report] 使用 Ollama 原生 API /api/chat, model={model}")
            content = self._ollama_chat(prompt, llm_config)
        else:
            # OpenAI 兼容接口
            print(f"[_generate_report] 使用 OpenAI 兼容接口, model={model}")
            response = llm_client.chat.completions.create(
                model=model,
                messages=[{"role": "user", "content": prompt}],
                temperature=0.2
