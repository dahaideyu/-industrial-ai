...
REM ============================================
REM 超威agent 一键构建脚本 —— 应用镜像 + PostgreSQL(TimescaleDB) 镜像
REM 一起打进一个 tar，供离线拷贝到服务器 docker load。
REM 用法: build.bat [版本/tag]
REM 示例: build.bat 1.0.0
REM 镜像名固定为 industrial-ai（必须跟 docker-compose.yml 里
REM industrial-ai 服务的 image: 字段一致，否则 docker load 之后
...
REM 1. 构建应用镜像（%~dp0 锚定本脚本所在目录，不依赖调用者当前工作目录）
echo [1/4] 构建应用 Docker 镜像（含 Cython 编译）...
docker build -f "%~dp0Dockerfile" --build-arg COMPILE_SOURCE=true --build-arg VITE_ENABLE_SQL_QA=true -t %FULL_IMAGE% "%~dp0..\.."
if errorlevel 1 (
    echo 错误: 镜像构建失败
    pause
    exit /b 1
...
REM 2. 拉取 PostgreSQL/TimescaleDB 官方镜像（本地已有相同 tag 则跳过）
...
REM 4. 读取密钥文件（由 encrypt_prompts.py 生成）
set "ENCRYPT_KEY="
if exist "%KEY_FILE%" (
